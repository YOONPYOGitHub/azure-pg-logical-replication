...
> CDC 단계에서 수동 추가한 테이블의 시퀀스는 Cutover 대상에 포함되지 않음. (상세: [§4 시퀀스 동기화 결과](#시퀀스-동기화-결과))
...
|------|---------------------------|----------|--------|
| **Logical Replication** | 에러로 차단 (안전) | ✅ 즉시 | ❌ 수동 `setval()` |
| **Azure Migration Service** | 조용히 누락 (위험) | ❌ 모름 | ⚠️ Cutover 시 조건부 자동 ¹ |

> ¹ 초기 Migration 생성 시 존재했던 테이블의 시퀀스만 Cutover 시 자동 동기화. CDC 중 수동 추가된 시퀀스는 제외.

### 핵심

- **두 방식 모두** PK 없는 테이블 + REPLICA IDENTITY 미설정 시 UPDATE/DELETE 동기화에 문제 발생
- Logical Replication은 **에러로 알려주지만**, Azure Migration Service는 **조용히 누락**
- **Migration 전에 반드시 PK 없는 테이블을 점검하고 조치 필요**
