...
- adventureworks2: Azure DMS (Online Migration) 동기화
...
- **상태**: adventureworks Logical Replication만 활성 / adventureworks2 DMS 중단됨
...
### [NEW] Databases
- adventureworks (존재)
- adventureworks2 (없음 - DMS 중단 시 삭제됨)

### Step 1 결론
- Logical Replication 관련 구성 요소만 존재: `migration_sub` slot, `migration_pub` publication, `migration_sub` subscription
- Azure DMS 관련 구성 요소 전무: `azpgmigslot_*` 슬롯 없음, `pgcopydb` WAL sender 없음

---

## Step 2: Logical Replication + Azure DMS 동시 동작
- **시간**: 2026-03-24 07:33 (UTC)
- **상태**: adventureworks Logical Replication + adventureworks2 Azure DMS 동시 동작
...
### [NEW] Databases
- adventureworks (존재)
- adventureworks2 (존재 - DMS가 새로 생성함)

### Step 2 결론 - Step 1과 비교
| 구성 요소 | Step 1 (LR만) | Step 2 (LR + DMS) | 변화 |
|---|---|---|---|
...
| adventureworks2 publication | 없음 | 없음 | **변화 없음** (DMS는 publication 불필요) |
| `migration_sub` subscription | adventureworks 전용 | adventureworks 전용 | **변화 없음** |
| NEW adventureworks2 DB | 없음 | 존재 | **DMS가 생성** |

---

## Step 3: Logical Replication 중단 + Azure DMS만 동작
- **시간**: 2026-03-24 07:37 (UTC)
- **상태**: adventureworks subscription DISABLED / adventureworks2 DMS 동작 중
...
| adventureworks2 (DMS 동작) | `DMS_LIVE_V2_1~3` 3건 INSERT | **3 rows (정상 도착)** | DMS 정상 동작 확인 |

### Step 3 결론 - Step 2와 비교
| 구성 요소 | Step 2 (LR + DMS) | Step 3 (DMS만) | 변화 |
|---|---|---|---|
...
| adventureworks2 데이터 동기화 | 동작 | **동작** | DMS는 LR과 무관하게 독립 동작 |
...
## 최종 결론: Logical Replication vs Azure DMS (Online Migration)
...
| SEQUENCE 동기화 | **불가** (양쪽 별도 채번) |
| 엔진 내 확인 | `pg_replication_slots`, `pg_stat_replication`에서 확인 가능 |

### 차이점

| 항목 | Logical Replication | Azure DMS (Online Migration) |
|---|---|---|
...
| **초기 데이터 복사** | subscription 생성 시 자동 (copy_data=true) | DMS가 `pgcopydb`로 full copy 수행 |
| **NEW DB 생성** | 사용자가 직접 생성 | DMS가 자동 생성 |
...
| Step | 상태 | 검증 결과 |
|---|---|---|
| Step 1 | LR만 동작 | LR 구성 요소만 존재, DMS 흔적 없음 |
| Step 2 | LR + DMS 동시 동작 | DMS 추가 시 LR에 영향 없음, 각각 별도 slot/WAL sender 생성 |
| Step 3 | DMS만 동작 (LR 중단) | LR 중단 시 DMS에 영향 없음, 데이터 검증으로 독립성 확인 |

**데이터 동기화 검증 (Step 3):**
- adventureworks (LR 중단): old INSERT → new **미도착** (0 rows)
- adventureworks2 (DMS 동작): old INSERT → new **정상 도착** (3 rows)
